...
def manhattan(rating1, rating2):
    ...
    '''
    # jeff's modified 
    commons = set([x for x in rating1]) & set([x for x in rating2])
    total = len(commons)
    distance = sum([abs(rating1[x] - rating2[x]) for x in commons])
    if total == 0:
        #print -1
        return -1
    else:
...
def recommend(username, users):
    ...
    #print sorted(recommendations, key=lambda artistTuple: artistTuple[1], reverse = True)


if __name__ == '__main__':
    #manhattan(users['Bill'], users['Chan'])
